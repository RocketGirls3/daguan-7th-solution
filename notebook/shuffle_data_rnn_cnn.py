...
output_pro_dir = '../pro/prob_rnn_cnn_baseline4.csv'
output_dir = '../output/sub_rnn_cnn_baseline4.csv'
output_pkl_dir = '../feature/rnn_cnn.pkl'
shuffle_data_dir = '../input/new_data/shuffle_train.csv'
word2vec_article_dir = '../feature/word2vec_file/char600d.w2v'
...
if os.path.exists(shuffle_data_dir):
    ...
    shuffle_1_data = []
    shuffle_19_data = []
    shuffle_1_data_article = []
    shuffle_19_data_article = []
    ...
    label_1_article = original_train.loc[original_train['class']==1]['article'].values
    label_19_article = original_train.loc[original_train['class']==19]['article'].values
    ...
    for i in label_1_article:
        words = i.split(' ')
        rs=random.sample(words,len(words))
        rs = ' '.join(rs)
        shuffle_1_data_article.append(rs)

    for i in label_19_article:
        words = i.split(' ')
        rs=random.sample(words,len(words))
        rs = ' '.join(rs)
        shuffle_19_data_article.append(rs)

    df_label19 = pd.DataFrame(shuffle_19_data,columns=['word_seg'])
    df_label19['class'] = 19
    df_label19['article']=shuffle_19_data_article

    df_label1 = pd.DataFrame(shuffle_1_data,columns=['word_seg'])
    df_label1['class'] = 1
    df_label1['article']=shuffle_1_data_article
...
###################################################3
if os.path.exists(word2vec_article_dir):
    pass
else:
    print('start_train_word2vec on char')
    model = Word2Vec(size=600, window=10,max_vocab_size=500000, sg=1)
    train2 = all_data['article'].values
    train2 = [text_to_word_sequence(text) for text in tqdm(train2)]
    model.build_vocab(train2)
    model.train(train2, total_examples=model.corpus_count, epochs=3)
    model.save(word2vec_article_dir)
    print("word2vec train done")

max_features = 500000
vec_len = 600
EMBEDDING = word2vec_article_dir
model = word2vec.Word2Vec.load(EMBEDDING)
...
def get_model():
    ...
    emb_article = Embedding(char_embedding_matrix.shape[0], 600, weights=[char_embedding_matrix], trainable=False)(
        inparticle)
...
def kf_train(fold_cnt=3, rnd=1):
    ...
    for train_index, test_index in kf.split(original_train):
        ...
        shuffle_word_pad_article = tr_article_pad[original_train.shape[0]:]
        shuffle_word_pad_y = one_y_train[original_train.shape[0]:]
        for shuffle_train_index, shuffle_test_index in kf.split(shuffle_data):
            shuffle_train_x1 = shuffle_word_pad[shuffle_train_index]
            shuffle_train_x2 = shuffle_word_pad_article[shuffle_train_index]
            shuffle_train_y = shuffle_word_pad_y[shuffle_train_index]
            if shuffle_number == now_nfold:
                break
            shuffle_number += 1
        curr_x1 = np.vstack((curr_x1, shuffle_train_x1))
        curr_x2 =  np.vstack((curr_x2, shuffle_train_x2))
        ...
        file_path = "weights_basercnn.best.h5"
